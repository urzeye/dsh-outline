/**
 * dsh-outline client half: registers the outline dictionaries and the
 * floating panel (shell.overlay, additive list seat, root scope). The panel
 * owns its own edge trigger (hover to peek, pin to keep) — no session-header
 * toggle. Value imports stay inside the loader module table (react) — every
 * @deepseek-ai import here is type-only and erased at build.
 */
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'outline: dictionaries')
